...
export default function (src, customClasses = {}, bootstrapStyled = getComputedStyle(document.documentElement).getPropertyValue('--bs-primary') != '') {
	...
	const element = (tag, content, attributes = {}) =>
		`<${tag} ${Object.entries(Object.assign(attributes, { class: classes(tag) }))
	...
	const highlight = (src) =>
		src.replace(rx.highlight, (all, _, p1, emp, sub, sup, small, big, p2, content) => _ + element(emp ? (p2 ? 'strong' : 'em') : sub ? (p2 ? 's' : 'sub') : sup ? 'sup' : small ? 'small' : big ? 'big' : 'code', highlight(content)));
	...
	src = src.replace(rx.link, (all, p1, p2, p3, p4, p5, p6) => {
		stash[--si] = p4 ? (p2 ? `<img src="${p4}" alt="${p3}"/>` : element('a', unesc(highlight(p3)), { href: p4 })) : p6;
		return si + '\uf8ff';
	});
			...
			element(
				...
				table.replace(rx.row, (row, ri) => {
					return row == sep
						...
						: element(
								'tr',
								row.replace(rx.cell, (all, cell, ci) => {
